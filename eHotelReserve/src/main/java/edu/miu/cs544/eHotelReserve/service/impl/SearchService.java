...
public class SearchService implements ISearchService {
	...
	// private IBookingRepository bookingRepository; String baseUrl =
	// "http://localhost:8000/MemberRest/roomTypes";
	...
	// @Autowired
	// public SearchService(IRoomRepository roomRepository, IBookingRepository
	// bookingRepository) {
	...
	public List<Room> getAvailableRooms(LocalDate checkIn, LocalDate checkOut) {
		...
		// ResponseEntity<Booking[]> responseEntity=restTemplate.exchange(baseUrl,
		// HttpMethod.GET, httpEntity, Booking[].class);
		// List<Booking> allBookings = Arrays.asList(responseEntity.getBody());

		// String result =
		// restTemplate.getForObject("http://example.com/hotels?state={state}&country={country}",
		// String.class,"NY","USA");
		ResponseEntity<Room[]> responseEntity = restTemplate.exchange(baseUrlExtended, HttpMethod.GET, httpEntity,
		...
		//
		// List<Room> booked = allBookings.stream().filter(b ->
		// (b.getCheckInDate().isBefore(checkIn) &&
		// b.getCheckOutDate().isAfter(checkOut)) ||
		// (b.getCheckInDate().isAfter(checkIn) &&
		...
		// b.getCheckOutDate().equals(checkOut))).map(b ->
		// b.getRoom()).collect(Collectors.toList());
		//
	...
	public List<RoomType> findAvailableRoomTypes(LocalDate checkIn, LocalDate checkOut) {
		...
		ResponseEntity<RoomType[]> responseEntity = restTemplate.exchange(baseUrlExtended, HttpMethod.GET, httpEntity,
				RoomType[].class, checkIn, checkOut);
